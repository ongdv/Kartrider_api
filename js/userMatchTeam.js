...
// 팀전
function onClickSpeedTeam(){
  const nickname = getID('userNickname').innerHTML;
  const matchType = getID('speedTeam').value;
  getMatchTeam(nickname, matchType);
}


// 클럽전
function onClickSpeedClub(){
  const nickname = getID('userNickname').innerHTML;
  const matchType = getID('speedClub').value;
  getMatchTeam(nickname, matchType);
}


// 클럽전, 팀전 정보를 받아옴
function getMatchTeam(nickname, matchType){
  console.log('getTeamMatch');
        ...
        matches.forEach((match) => {
          ...
          <div class="detailInfoBox">
           <div id="detailInfoBox" class="detailInfo">
            ...
            <button id="myMatch" class="detailUsersButton" onclick="onClickDetailUsersTeam(\'${matchId}\', \'${i}\')">▼</button>
            <form method="GET" action="/userTeamMatchDetail/matchId">
 ...
 * 대전 상세 정보를 가져온다.
 */
function onClickDetailUsersTeam(matchId, index) {
  ...
  if (isClicked[index]) {
    ...
    if (!isRequested[index]) {
      ...
      fetch(`/userTeamMatchDetail/matchId?matchId=${matchId}`, {
        method: 'GET',
        headers: { 'Content-Type': 'application/json' },
      })
        .then((res) => {
